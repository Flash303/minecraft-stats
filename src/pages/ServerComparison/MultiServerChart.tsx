...
export function MultiServerChart({ data, serverNames, timeRange }: MultiServerChartProps) {
        ...
        return {
            hooks: {
                ...
                setCursor: (u: uPlot) => {
                        ...
                        if (yVal !== null && yVal !== undefined) {
                            ...
                            rowsHtml += `
                                <div class="flex items-center justify-between gap-4 py-0.5">
                                    ...
                                    <span class="font-bold">${new Intl.NumberFormat("fr-FR").format(Math.round(yVal))}</span>
                                </div>
                            `
                        }
                    }
                    ...
                    overlay.innerHTML = `
                        <div class="border-b border-white/10 pb-1 mb-1 text-slate-400">📅 ${dateStr} à ${timeStr}</div>
                        ${rowsHtml}
                    `
    ...
    }, [serverNames, theme, tooltipPlugin, timeRange])

    if (data[0].length === 0) {
        return <p className="text-center py-4 text-slate-400">Aucune donnée disponible pour les serveurs sélectionnés</p>
    }
